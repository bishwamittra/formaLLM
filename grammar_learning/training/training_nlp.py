...
def training(args, dataset_dict, max_sequence_length, unique_tokens):
    ...
    if local_rank == 0:
        ...
        for eval_dataset in dataset:
            ...
            print(type(dataset[eval_dataset]["input_ids"]))
            
            # for i in range(len(dataset[eval_dataset]["input_ids"])):
            #     print(len(dataset[eval_dataset]["input_ids"][i]))
    ...
    if args.inference_only_mode:
        ...
        if args.incontext_input:
            ...
            if len(args.considered_incontext_examples[1]) == 0:
                ...
                if args.nlp_dataset:
                    incontext_common_prefix_len = len(tokenizer.encode(args.considered_incontext_examples[1]))
                else:   
